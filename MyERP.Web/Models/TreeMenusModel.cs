...
{
    public class TreeMenusModel
    {
        internal static string TreeMenusRoot = "~/Areas/";

        public static NodeCollection GetTreeMenusNodes()
        {
            var nodes = new NodeCollection();
            string path = HttpContext.Current.Server.MapPath(TreeMenusModel.TreeMenusRoot);

            return TreeMenusModel.BuildAreasLevel();
        }
        ...
        private static NodeCollection BuildAreasLevel()
        {
            ...
            folders = TreeMenusModel.SortFolders(root, folders);
            ...
            foreach (DirectoryInfo folder in folders)
            {
                ...
                TreeMenuConfig cfg = new TreeMenuConfig(folder.FullName + "\\config.xml");
                ...
                TreeMenusModel.BuildViewsLevel(folder, node);
            }
        ...
        private static void BuildViewsLevel(DirectoryInfo area, Node areaNode)
        {
            ...
            folders = TreeMenusModel.SortFolders(area, folders);
            ...
            foreach (DirectoryInfo folder in folders)
            {
                ...
                node.IconCls = iconCls;
                string url = string.Concat(TreeMenusModel.ApplicationRoot, "/", area.Name, "/", folder.Name, "/");
        ...
        private static TreeMenuConfig rootCfg;
        ...
        {
            if (TreeMenusModel.rootCfg == null)
            {
                TreeMenusModel.rootCfg = new TreeMenuConfig(new DirectoryInfo(HttpContext.Current.Server.MapPath(TreeMenusModel.TreeMenusRoot)) + "\\config.xml");
            }

            foreach (string newFolder in rootCfg.NewFolders)
            {
                string newPath = string.Concat(HttpContext.Current.Server.MapPath(TreeMenusModel.TreeMenusRoot), newFolder);
        ...
        private static bool IsUpdated(string folder)
        {
            if (rootCfg == null)
            {
                rootCfg = new TreeMenuConfig(new DirectoryInfo(HttpContext.Current.Server.MapPath(TreeMenusModel.TreeMenusRoot)) + "\\config.xml", false);
            }

            foreach (string updFolder in rootCfg.UpdFolders)
            {
                string updPath = string.Concat(HttpContext.Current.Server.MapPath(TreeMenusModel.TreeMenusRoot), updFolder);
        ...
        private static DirectoryInfo[] SortFolders(DirectoryInfo root, DirectoryInfo[] folders)
        {
            ...
            {
                TreeMenuConfig rootCfg = new TreeMenuConfig(cfgPath);
    ...
    public class TreeMenuConfig
    {
        ...
        public static string PhysicalToVirtual(string physicalPath)
        {
            ...
            return TreeMenuConfig.ApplicationRoot + relUrl;
        }

        private string path;
        private bool includeDescriptors;

        public TreeMenuConfig(string path, bool includeDescriptors = false)
        {
            this.path = path;
        ...
        private void Load()
        {
            ...
            folders = root.SelectNodes("new[@version = \"" + MyERPVersion.WithBuild + "\"]/folder");
            ...
            // This will match only new items for the current version a.b.c.
            folders = root.SelectNodes("updated[@version = \"" + MyERPVersion.WithBuild + "\"]/folder");
            ...
            if (files != null)
            {
                string url = TreeMenuConfig.PhysicalToAbsolute(path);
