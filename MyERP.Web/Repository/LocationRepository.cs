...
namespace MyERP.Web
{
    ...
    {
        public Paging<Location> Paging(StoreRequestParameters parameters)
        {
            return Paging(parameters.Start, parameters.Limit, parameters.SimpleSort, parameters.SimpleSortDirection, null);
        }

        public  Paging<Location> Paging(int start, int limit, string sort, SortDirection dir, string filter)
        {
            ...
            return new Paging<Location>(ranges, count);
        }
    }
