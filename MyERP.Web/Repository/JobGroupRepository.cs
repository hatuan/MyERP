...
namespace MyERP.Web
{
    public partial class JobGroupRepository 
    {
        public Paging<JobGroup> Paging(StoreRequestParameters parameters)
        {
            return Paging(parameters.Start, parameters.Limit, parameters.SimpleSort, parameters.SimpleSortDirection, null);
        }

        public Paging<JobGroup> Paging(int start, int limit, string sort, SortDirection dir, string filter)
        {
            ...
            return new Paging<JobGroup>(ranges, count);
        }
    }
