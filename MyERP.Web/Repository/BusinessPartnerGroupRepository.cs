...
namespace MyERP.Web
{
    public partial class BusinessPartnerGroupRepository
    {
        public Paging<BusinessPartnerGroup> Paging(StoreRequestParameters parameters)
        {
            return Paging(parameters.Start, parameters.Limit, parameters.SimpleSort, parameters.SimpleSortDirection, null);
        }

        public Paging<BusinessPartnerGroup> Paging(int start, int limit, string sort, SortDirection dir, string filter)
        {
            ...
            return new Paging<BusinessPartnerGroup>(ranges, count);
        }
    }
