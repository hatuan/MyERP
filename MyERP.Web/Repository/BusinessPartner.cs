...
namespace MyERP.Web
{
    public partial class BusinessPartnerRepository
    {
        public Paging<BusinessPartner> Paging(StoreRequestParameters parameters)
        {
            return Paging(parameters.Start, parameters.Limit, parameters.SimpleSort, parameters.SimpleSortDirection, null);
        }

        public Paging<BusinessPartner> Paging(int start, int limit, string sort, SortDirection dir, string filter)
        {
            var entities = Get(includePaths: new String[] { "Organization", "Client", "RecCreatedByUser", "RecModifiedByUser", "BusinessPartnerGroup1", "BusinessPartnerGroup2", "BusinessPartnerGroup2" });
            ...
            return new Paging<BusinessPartner>(ranges, count);
        }
    }
