...
namespace MyERP.Web
{
    public partial class SalesPriceGroupRepository
    {
        public Paging<SalesPriceGroup> Paging(StoreRequestParameters parameters)
        {
            return Paging(parameters.Start, parameters.Limit, parameters.SimpleSort, parameters.SimpleSortDirection, null);
        }

        public Paging<SalesPriceGroup> Paging(int start, int limit, string sort, SortDirection dir, string filter)
        {
            var entities = Get(includePaths: new String[] { "Organization" });
            ...
            return new Paging<SalesPriceGroup>(ranges, count);
        }
    }
