...
namespace MyERP.Web
{
    public partial class BusinessPartnerPriceGroupRepository
    {
        public Paging<BusinessPartnerPriceGroup> Paging(StoreRequestParameters parameters)
        {
            return Paging(parameters.Start, parameters.Limit, parameters.SimpleSort, parameters.SimpleSortDirection, null);
        }

        public Paging<BusinessPartnerPriceGroup> Paging(int start, int limit, string sort, SortDirection dir, string filter)
        {
            ...
            return new Paging<BusinessPartnerPriceGroup>(ranges, count);
        }
    }
